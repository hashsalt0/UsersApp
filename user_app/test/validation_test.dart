...
void main() {
  ...
  test('Age Vaildation Test', () {
    expect(() => Validations.ageValidation(-1),
        throwsA(TypeMatcher<InputException>()));
    expect(() => Validations.ageValidation(null),
        throwsA(TypeMatcher<InputException>()));
    expect(() => Validations.ageValidation(0),
        throwsA(TypeMatcher<InputException>()));
    expect(Validations.ageValidation(10), true);
  });
  test('Address Vaildation Test', () {
    expect(() => Validations.addressValidation(""),
        throwsA(TypeMatcher<InputException>()));
  ...
  test('Roll No Vaildation Test', () {
    ...
    Store.instance.add(UserModel.fromJson({
      Const.rollNumberSerial: 2,
      Const.coursesSerial: [],
    }));
    expect(() => Validations.rollNumberValidation(2),
        throwsA(TypeMatcher<RollNumberAlreadyExistsException>()));
  });

  test('Courses Vaildation Test', () {
    expect(Validations.coursesValidation(Course.A, HashSet()), true);
    expect(
        () => Validations.coursesValidation(Course.A, HashSet.from([Course.A])),
        throwsA(TypeMatcher<CourseAlreadyAddedException>()));
  });
}
